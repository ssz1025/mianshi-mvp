...
-- 4. 插入题目
INSERT INTO question (id, title, content, difficulty, is_vip, view_count, star_count, like_count, creator_id) VALUES
(1, 'HashMap 的底层实现原理是什么？', 'HashMap 是基于哈希表实现的，JDK 1.8 之后采用数组+链表+红黑树的数据结构。当链表长度超过 8 且数组长度超过 64 时，链表会转换为红黑树以提高查询效率。put 过程：先计算 hash 值定位桶位置，如果桶为空直接插入，否则遍历链表或红黑树进行更新或插入。扩容时默认容量翻倍，重新计算每个元素的位置。', 2, false, 9856, 3200, 2100, 1),
(2, 'MySQL 索引的底层数据结构为什么用 B+ 树？', 'B+ 树相比 B 树和二叉树，具有更矮的树高，磁盘 I/O 次数更少。叶子节点通过链表相连，支持范围查询。非叶子节点只存储键值，单个节点能存储更多键值，从而降低树的高度。B+ 树的查询性能稳定，每次查询都需要走到叶子节点，路径长度相同。', 2, false, 8734, 2800, 1900, 1),
(3, 'Redis 为什么这么快？', 'Redis 基于内存操作，使用单线程避免上下文切换和锁竞争，采用 IO 多路复用模型（epoll），高效的数据结构设计（如 SDS、跳表、压缩列表、整数集合等）。此外 Redis 还有多线程 I/O 读写（6.0+），但命令执行仍是单线程。', 2, false, 8120, 2600, 1800, 1),
(4, 'Spring Boot 自动配置原理是什么？', 'Spring Boot 通过 @EnableAutoConfiguration 注解开启自动配置，利用 SpringFactoriesLoader 加载 META-INF/spring.factories 中的配置类，根据 @Conditional 系列注解（@ConditionalOnClass、@ConditionalOnMissingBean 等）的条件判断来决定是否生效。核心是 spring-boot-autoconfigure 模块。', 3, false, 7650, 2400, 1600, 1),
(5, 'TCP 三次握手和四次挥手的过程？', '三次握手：客户端发送 SYN=1, seq=x → 服务端回复 SYN=1, ACK=1, seq=y, ack=x+1 → 客户端发送 ACK=1, seq=x+1, ack=y+1。四次挥手：主动方发送 FIN → 被动方回复 ACK → 被动方发送 FIN → 主动方回复 ACK 并进入 TIME_WAIT 状态等待 2MSL。', 2, false, 7230, 2200, 1500, 1),
(6, '线程池的核心参数有哪些？', 'corePoolSize 核心线程数、maximumPoolSize 最大线程数、keepAliveTime 空闲线程存活时间、unit 时间单位、workQueue 工作队列、threadFactory 线程工厂、handler 拒绝策略（AbortPolicy、CallerRunsPolicy、DiscardPolicy、DiscardOldestPolicy）。执行流程：核心线程 → 队列 → 非核心线程 → 拒绝策略。', 2, false, 6890, 2100, 1400, 1),
(7, 'JVM 垃圾回收算法有哪些？', '标记-清除算法：产生内存碎片。复制算法：新生代使用，将内存分为 Eden 和两个 Survivor 区，效率高但浪费空间。标记-整理算法：老年代使用，解决碎片问题。分代收集算法：根据对象存活时间分代，不同代使用不同算法。常见垃圾收集器：Serial、Parallel、CMS、G1、ZGC。', 3, true, 6540, 1900, 1200, 1),
(8, '什么是 CAP 定理？', 'CAP 定理指出分布式系统不可能同时满足一致性(Consistency)、可用性(Availability)和分区容错性(Partition tolerance)，最多只能同时满足其中两个。由于网络分区必然存在，所以实际是在 CP 和 AP 之间选择。ZooKeeper 是 CP 系统，Eureka 是 AP 系统。', 2, false, 6120, 1800, 1100, 1),
(9, 'React Hooks 的使用注意事项？', '只能在函数组件顶层调用 Hook，不能在循环、条件或嵌套函数中调用。自定义 Hook 必须以 use 开头。useEffect 的依赖数组需要正确设置，避免闭包陷阱。useCallback 和 useMemo 用于性能优化。useRef 用于保存可变值且不触发重渲染。', 2, false, 5780, 1700, 1000, 1),
(10, '消息队列如何保证消息不丢失？', '生产者端：使用确认机制(confirm)、本地消息表保障消息发出。MQ端：持久化存储、集群部署、主从复制。消费者端：手动ACK确认、幂等性处理防止重复消费。RocketMQ 的事务消息可以保证分布式事务的最终一致性。', 2, false, 5430, 1600, 900, 1),
(11, 'ArrayList 和 LinkedList 的区别？', 'ArrayList 基于动态数组实现，随机访问 O(1)，插入删除需要移动元素 O(n)。LinkedList 基于双向链表实现，插入删除 O(1)（已知位置），随机访问 O(n)。ArrayList 内存占用更紧凑，LinkedList 每个节点额外存储前后指针。实际开发中 ArrayList 使用更多。', 1, false, 5120, 1500, 850, 1),
(12, '什么是乐观锁和悲观锁？', '乐观锁假设冲突概率低，通过版本号或 CAS（Compare And Swap）实现，适合读多写少场景，如数据库 version 字段。悲观锁假设冲突概率高，通过数据库 SELECT FOR UPDATE 或 Java synchronized 实现，适合写多场景。乐观锁可能产生 ABA 问题，可用 AtomicStampedReference 解决。', 1, false, 4890, 1400, 800, 1),
(13, 'HTTP 和 HTTPS 的区别？', 'HTTPS 在 HTTP 基础上增加了 SSL/TLS 加密层，数据传输加密防止窃听篡改。HTTPS 需要 CA 证书，默认端口 443（HTTP 为 80）。HTTPS 握手过程更复杂，需要密钥交换和身份验证。TLS 1.3 简化了握手流程，支持 0-RTT。', 1, false, 4650, 1300, 750, 1),
(14, 'Vue 和 React 的区别？', 'Vue 采用模板语法，React 采用 JSX。Vue 数据双向绑定(MVVM)，React 单向数据流。Vue 使用 Composition API，React 使用 Hooks。Vue 内置路由和状态管理，React 需要第三方库。Vue 更易上手，React 生态更丰富。两者都支持虚拟 DOM 和组件化开发。', 2, false, 4420, 1200, 700, 1),
(15, '快速排序的时间复杂度？', '平均时间复杂度 O(nlogn)，最坏情况 O(n²)（数组已排序时）。空间复杂度 O(logn)（递归栈）。是不稳定排序。可通过随机化选择基准元素避免最坏情况。三路快排可以优化大量重复元素的情况。实际应用中通常结合插入排序，小数组时切换。', 2, false, 4180, 1100, 650, 1);
